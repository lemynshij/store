class ProductsController < ApplicationController
  ...
  def index
    ...
    unless @featured_products
      @featured_products = Product.includes(:product_images).where(['category_id = 272']).order('msrp ASC').limit(8)
      #Cache.put 'featured_prod_8', @featured_products
    end
    #@new_products = Cache.get 'new_prod_8'
    unless @new_products
      @new_products = Product.includes(:product_images).limit(8).order('created_at DESC')
      #Cache.put 'new_prod_8', @new_products
    end
    #@top_sellers = Cache.get 'top_seller_8'
    unless @top_sellers
      @top_sellers  = Product.includes(:product_images).where([ 'category_id in (564, 567)']).order('msrp, created_at DESC').limit(8)
      #Cache.put 'top_seller_8', @top_sellers
    end
    #@recommended = Cache.get 'recommended_8'
    unless @recommended
      @recommended = Product.includes(:product_images).where([ 'category_id = 274']).order('msrp, created_at DESC').limit(8)
